...
public class Snake extends Entity
{
    ...
    public void executeDecision()
    {
        ...
        if(currentDirection == UP && arg == DOWN) {return;}
        if(currentDirection == DOWN && arg == UP) {return;}
        if(currentDirection == LEFT && arg == RIGHT) {return;}
        if(currentDirection == RIGHT && arg == LEFT) {return;}
        currentDirection = arg;
    }

    public void loadInputs()
    {
        loadSensors();
        inputVector[3][0][0] = velocity[0];
        inputVector[4][0][0] = velocity[1];
        inputVector[5][0][0] = (pellet.getX()) - (position[0]);
        inputVector[6][0][0] = (pellet.getY()) - (position[1]);
    }
