...
def build_models(inputShape,numROIs,numAnchors,numClasses):
    ...
    base = simple_base(input_tensor = imgInput)
...
def build_test_models(inputShape,mapInputShape,ROIInputShape,numROIs,numAnchors,numClasses,trainable=False):
    inputTensor = Input(shape=inputShape)
    ROIInput = Input(shape=ROIInputShape)
    mapInput = Input(shape=mapInputShape)
    
    base = resnet_base(input_tensor = inputTensor,trainable=False)
    rpnLayers = rpn(base,numAnchors)
    classifier = get_classifier(mapInput,ROIInput,numROIs,nb_classes=numClasses,trainable=trainable)

    rpnModel = Model(inputs=inputTensor,outputs=rpnLayers)
    classifierModel = Model(inputs=[mapInput,ROIInput],outputs=classifier)
    classifierOnly = Model(inputs=[mapInput,ROIInput],outputs=classifier)
    return [rpnModel, classifierModel, classifierOnly]
def simple_base(input_tensor=None, trainable=True):
