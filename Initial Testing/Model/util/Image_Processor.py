...
class SampleSelector:
	def __init__(self, class_count):
		...
		self.classes = [b for b in list(class_count.keys()) if class_count[b] > 0]
...
def calc_rpn(C, img_data, width, height, resized_width, resized_height):
	...
	if len(pos_locs[0]) > num_regions/2:
		val_locs = random.sample(list(range(len(pos_locs[0]))), len(pos_locs[0]) - num_regions/2)
		y_is_box_valid[0, pos_locs[0][val_locs], pos_locs[1][val_locs], pos_locs[2][val_locs]] = 0
		num_pos = num_regions/2

	if len(neg_locs[0]) + num_pos > num_regions:
		val_locs = random.sample(list(range(len(neg_locs[0]))), len(neg_locs[0]) - num_pos)
	...
	while True:
		...
		for img_data in all_img_data:
			try:
				...
				img_data_aug['width'] = x_img.shape[1]
				img_data_aug['height'] = x_img.shape[0]
				...
				try:
					...
					print(("EXCEPTION IN IMAGE PROCESSOR CALCULATING RPN OUTPUTS |",type(e).__name__,"|",e.args))
					continue
				...
				print(("EXCEPTION IN IMAGE PROCESSOR WHILE LOOP |",type(e).__name__,"|",e.args))
				continue
