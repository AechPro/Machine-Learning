...
public class Layer
{
    ...
    public Layer(int numNodes, boolean isInput, Layer prev)
    {
        inputLayer = isInput;
        prevLayer = prev;
        nodes = numNodes;
    ...
    public void activateNoisy(double[] noiseFlat, int offset, boolean negative)
    {
        ...
        for(int i=0;i<weights.length;i++)
        {
            for(int j=0;j<weights[i].length;j++)
            {
                ...
                if(negative)
                {
                    noisyWeights[i][j] = weights[i][j] - noiseFlat[offset + idx++];
                }
                else
                {
                    noisyWeights[i][j] = weights[i][j] + noiseFlat[offset + idx++]; 
    ...
    private void buildWeights()
    {
        ...
        for(int i=0;i<weights.length;i++)
        {
            for(int j=0;j<weights[i].length;j++)
            {
                biases[i] = Rand.getRandNorm(0, Config.NOISE_STD_DEV);
                weights[i][j] = Rand.getRandNorm(0, Config.NOISE_STD_DEV);
            }
        }
