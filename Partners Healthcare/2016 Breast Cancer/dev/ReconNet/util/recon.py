...
class Reconstruction(object):
    ...
    def compute(self, data):
        ...
        if self.UpsampleFactor > 0:
            subNormAmp, d2 = self.upsampling(subNormAmp, self.delta2)
        ...
        """supportVis = support.astype('float32')
        supportVis*=255
        supportVis = supportVis.astype('uint8')
        cv2.imshow("support",supportVis)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        cv2.imwrite("object_support.png",supportVis)"""
...
for file in os.listdir(dir):
    if ".png" in file:
        ...
        refName = ''.join([dir, '/',file])
        result, support = recon.process(fileName, refName)
        cv2.imshow("recon",np.abs(result))
        cv2.moveWindow("recon",300,200)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        print("Reconstruction runs in:", time.time() - t1, "seconds")
fileName = "../images/current samples/snapshot/image_snapshot.png"
refName = "../images/current samples/snapshot/ref_snapshot.png"
result = recon.process(fileName, refName)
cv2.imshow("recon",np.abs(result))
cv2.moveWindow("recon",300,200)
cv2.waitKey(0)
cv2.destroyAllWindows()
