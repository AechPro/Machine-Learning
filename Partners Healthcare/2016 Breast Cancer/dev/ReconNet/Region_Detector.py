...
def get_regions(sourceImage, reference, cellName="a0", imName="test.png", flter=None, show=False, classify=False):
    ...
    regions, cells, time = extract_regions(sourceImage, reference, params, imName, cellName=cellName, flter=flter, show=show,
                                     classify=classify)
...
def name_region(name):
    ...
    return name
"""
dir = "../images/current samples/2. blue"
fileName = ''.join([dir, "/BT474_blue_top right_625.png"])
refName = ''.join([dir, '/ref_625.png'])
img = cv2.imread(fileName,cv2.IMREAD_ANYDEPTH)
ref = cv2.imread(refName,cv2.IMREAD_ANYDEPTH)
params = [3, 101, 226, 0.7214, 0.3703, 375, 4, 0.5034, 11]
regions, cells, _ = extract_regions(img, ref, params, "genome_test", cellName="a0", flter=None, show=True,
                                     classify=True)"""